    ...
    return (
      <View className="flex-1 bg-bgCanvas">
        ...
        <View className="flex-1 items-center justify-center" style={{ paddingTop: insets.top + 16 }}>
          <Text className="font-body text-base text-textSecondary mb-4">No outfit combinations available</Text>
          <Pressable onPress={handleClose} className="bg-goldAccent px-6 py-3 rounded-full">
            <Text className="font-ui text-sm font-medium text-white">Close</Text>
          </Pressable>
        </View>
      </View>
    );
    ...
    <View className="flex-1 bg-bgCanvas">
      ...
      <View className="flex-row items-center justify-between px-4 pb-4 z-10" style={{ paddingTop: insets.top + 8 }}>
        ...
        <Pressable onPress={handleBookmark} className="w-11 h-11 rounded-full bg-white/80 items-center justify-center">
          <View className="w-5 h-4 justify-between">
            <View className="w-5 h-0.5 bg-textPrimary rounded-full" />
            <View className="w-5 h-0.5 bg-textPrimary rounded-full" />
            <View className="w-5 h-0.5 bg-textPrimary rounded-full" />
          </View>
        </Pressable>
      ...
      {!isFaceSheetOpen && (
        <View className="items-center py-2 z-10">
          <Text className="font-body text-xs text-textSecondary bg-white/80 px-2 py-1 rounded-lg">
      ...
      <View 
        className="absolute bottom-0 left-0 right-0 flex-row items-center justify-center px-4 gap-3 bg-bgCanvas z-10"
        style={[
          { paddingBottom: insets.bottom + 16 },
          isFaceSheetOpen && { opacity: 0, pointerEvents: 'none' }
        ]}
      >
        <Pressable className="flex-row items-center justify-center px-6 py-3.5 rounded-full gap-2 bg-goldAccent" onPress={handleTryOn}>
          <Sparkles size={18} color={THEME.bgSurface} />
          <Text className="font-ui text-sm font-medium text-white">Try on</Text>
        </Pressable>

        <Pressable className="flex-row items-center justify-center px-6 py-3.5 rounded-full gap-2 bg-white" onPress={handleEdit}>
          <Edit3 size={18} color={THEME.textPrimary} />
          <Text className="font-ui text-sm font-medium text-textPrimary">Edit</Text>
        </Pressable>
